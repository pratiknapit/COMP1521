...
find_student_by_id__loop__body:
	...
	la	$t1, students					# $t1 = students[0]
	mul	$t2, $t0, SIZEOF_STRUCT_STUDENT			
	add	$t1, $t1, $t2					# $t1 = students[i]
	lw	$t3, STRUCT_STUDENT_ID_OFFSET($t1)		# $t3 = students[i].id


	bne	$t3, $a0, find_student_by_id__loop__step	# if (students[i].id == id) {
	move	$v0, $t1					# return &students[i]
...
update_student_mark__body:
	...
	li	$v0, 4
	la	$a0, update_student_mark__id_prompt		# printf("Please enter the student ID: ");
	syscall
...
call_find_student_by_id:					# struct student *student = find_student_by_id(id);
	move 	$a0, $v0
	jal 	find_student_by_id

if_statement:
	move	$t2, $v0					# store address student at $t2
	bne	$v0, NULL, end_if				# if (student == NULL) {
	
	li	$v0, 4						# printf("Student not found in class!\n");
...
end_if:
	...
	sw	$v0, STRUCT_STUDENT_MARK_OFFSET($t2)		# students[i].mark = entered value
...
print_report__loop__body:
	...
	mul	$t2, $t0, SIZEOF_STRUCT_STUDENT			
	add	$t1, $t1, $t2					# $t1 = students[i]
	lw	$a0, STRUCT_STUDENT_ID_OFFSET($t1)		# $a0 = students[i].id
	li	$v0, 1						# printf("%d\t", students[i].id)
	syscall
	...
	lw	$t2, STRUCT_STUDENT_MARK_OFFSET($t1)		# $t2 = students[i].mark
if: 
	bne	$t2, UNKNOWN_MARK, else
	li	$a0, '?'					# printf("?\n")
	li	$v0, 11
	syscall
	li	$a0, '\n'
